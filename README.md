...
The JSON schema represents the technical specification for a token list which can be used in a dApp interface, such as the Starkdefi Interface.

## What are token lists?

Starkdefi Token Lists is a specification for lists of token metadata (e.g. address, decimals, ...) that can be used by any dApp interfaces that needs one or more lists of tokens.
...
Token list JSON must validate against the [JSON schema](https://json-schema.org/) in order to be used in the Starkdefi Interface.
Tokens on token lists, and token lists themselves, are tagged so that users can easily find tokens.

## JSON Schema $id

The JSON schema ID is [https://starkdefi.org/tokenlist.schema.json](https://starkdefi.org/tokenlist.schema.json)
...
import { schema } from '@starkdefi/token-lists'
import Ajv from 'ajv'
...
```typescript
import { TokenList, schema } from '@starkdefi/token-lists'
...
A snapshot of the Starkdefi default list encoded as a token list is found in [test/schema/bigexample.tokenlist.json](test/schema/bigexample.tokenlist.json).
